...
Pull requests, `main`, and the weekly scheduled workflow pull the pinned base before building. The scheduled workflow disables BuildKit cache so stale layers cannot hide package or scanner changes. Release candidates also use a clean, pull-enabled build on native AMD64 and ARM64 runners. Each native release runner pushes its candidate by content digest with attestations, removes Docker Hub credentials, and then tests and scans that exact digest. The publisher does not rebuild the image; it only combines the two validated digest artifacts into the version and `latest` indexes. After manifest verification and Docker logout, it checks out the immutable release tag so `release-creator` can generate notes from Git history without exposing registry credentials to repository content.
