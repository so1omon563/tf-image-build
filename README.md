...
Pull requests and updates to `main` run static checks, then build, exercise, and vulnerability-scan the complete runtime contract on native Linux AMD64 and Linux ARM64 GitHub-hosted runners. A weekly scheduled run performs clean builds with a fresh base pull but never tags or publishes an image. Every merged pull request receives an immutable patch tag. GitHub and Docker Hub publication is requested only when `#release`, `#publish`, or `#ship` appears in the merge commit message. For the repository's normal squash-merge flow, put the marker in the pull request title; a marker only in the pull request body does not reach the merge commit and will not publish. Release candidates must pass the same per-architecture tests and security gate from clean builds before publication. The trusted publisher pushes both variants under one manifest for the immutable version tag and the moving `latest` alias, with explicit SBOM and max-level provenance attestations. See [`SECURITY.md`](SECURITY.md) for the base lifecycle, scan policy, exception rules, and retained evidence.
